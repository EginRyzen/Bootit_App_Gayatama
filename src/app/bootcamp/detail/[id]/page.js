...
export default function Page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const docRef = doc(db, "bootcamp", id); // Mengakses dokumen berdasarkan Firestore ID
        ...
        setError("Error fetching Bootcamp details.");
        console.error(err);
      } finally {
        setLoading(false);
      }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row flex-wrap md:justify-between py-10 gap-3 relative z-20">
          <div className="w-full md:w-[60%]">
            <div className="text-lg md:text-2xl font-semibold text-[#3a3f47]">
              {item.title}
            </div>
            <p className="text-[#3a3f47] text-xs md:text-sm my-4">
              {item.description}
            </p>
            <div className="flex gap-10 mb-10">
              <div className="flex items-center pb-3">
                ...
                <p className="ms-2 text-xs font-bold text-gray-900">
                  {item.rating}
                </p>
                <span className="w-1 h-1 mx-1.5 bg-gray-500 rounded-full" />
                <div className="text-xs font-medium text-gray-900">
                  {item.students} Registered Students
                </div>
              </div>
              <a className="flex">
                <IoShareSocialSharp />
                <span className="text-xs font-medium text-[#3a3f47]">
                  Share
                </span>
              </a>
            </div>
            <div className="text-lg md:text-xl font-semibold text-[#3a3f47]">
              About the Bootcamp Class
            </div>
            <p className="text-[#3a3f47] text-xs md:text-sm my-3">
              In this online class, you will learn {item.title}. The study
              materials are structured to help you understand them easily.
            </p>
            <div className="text-lg md:text-xl mt-10 font-semibold text-[#3a3f47]">
              Course Content
            </div>

            <div id="accordion-flush">
              {item.modul &&
                item.modul.map((modul, index) => (
                  <div key={index}>
            ...
            <div className="text-lg md:text-xl mt-10 mb-5 font-semibold text-[#3a3f47]">
              Presenter
            </div>
            ...
            <div className="w-full max-w-sm backdrop-blur-sm bg-white/30 border border-gray-200 rounded-lg shadow">
              <div className="flex flex-col items-center pb-10">
                <img
                  ...
                  alt="profile"
                />
                <h5 className="mb-1 text-lg md:text-xl font-medium text-gray-900">
                  {item.presenter}
                </h5>
                <span className="text-xs md:text-sm text-[#3a3f47]">
                  {item.jobPresenter}
                </span>
                <div className="mt-4 px-5 md:mt-6">
                  <p className="text-[#3a3f47] text-center text-xs md:text-sm">
                    {item.descPresenter}
                  </p>
                </div>
              </div>
          ...
          <div className="w-full md:w-[30%]">
            <div className="backdrop-blur-sm bg-white/30 border border-gray-200 rounded-lg shadow sticky top-10">
              ...
              <div className="p-5">
                <a href="#">
                  <h5 className="mb-2 text-lg md:text-xl font-semibold text-[#3a3f47]">
                    Buy through Roadmap
                  </h5>
                </a>
                <p className="text-[#3a3f47] font-normal text-xs md:text-sm">
                  What you will get:
                </p>
                <ul className="my-5">
                  <li className="flex items-center mb-2">
                    <FaSwatchbook className="size-4 text-slate-500" />
                    <p className="text-xs font-semibold text-[#3a3f47] ms-2 capitalize">
                      {item.module}
                    </p>
                  </li>
                  <li className="flex items-center mb-2">
                ...
                <a
                  href={item.link}
                  className="inline-flex items-center justify-center w-full text-[#3a3f47] hover:text-white hover:bg-[#3a3f47] border bg-white border-[#3a3f47] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium px-3 py-2 text-xs md:text-sm text-center rounded-lg"
                >
                  Join Now
                </a>
              </div>

"use client";

import React, { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import Search from "./components/serach.js";
import InputSelect from "./components/inputselect.js";
import Pagination from "./components/pagination.js";
import { db } from "./../../../firebase.js";
import { collection, getDocs } from "firebase/firestore";

function Page() {
  const router = useRouter();
  const [items, setItems] = useState([]);

  useEffect(() => {
    const fetchItems = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "meetup"));
        const items = querySnapshot.docs.map((doc) => ({
          ...doc.data(),
          id: doc.id,
        }));
        setItems(items);
      } catch (error) {
        console.error("Error fetching competitions data: ", error);
      }
    };

    fetchItems();
  }, []);

  const handleCardClick = (id) => {
    // Navigate to the detail page with the meetup id
    router.push(`meetup/detail/${id}`);
  };

  return (
    <main>
      <div className="fixed flex top-0 -z-10 h-full w-full bg-white">
        {/* Warna Ungu */}
        <div className="absolute bottom-auto left-auto right-0 top-0 h-[500px] w-[500px] -translate-x-[30%] translate-y-[20%] rounded-full bg-[rgba(173,109,244,0.5)] opacity-50 blur-[80px]"></div>

        {/* Warna Merah */}
        <div className="absolute bottom-auto left-auto right-0 top-0 h-[500px] w-[500px] translate-x-[10%] translate-y-[50%] rounded-full bg-[rgba(244,109,109,0.5)] opacity-50 blur-[80px]"></div>

        {/* Warna Biru */}
        <div className="absolute bottom-auto left-0 top-0 h-[500px] w-[500px] translate-x-[10%] translate-y-[20%] rounded-full bg-[rgba(95,164,242,0.5)] opacity-50 blur-[80px]"></div>
      </div>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row py-20 gap-10">
        <div className="w-full md:w-[30%]">
          <div className="sticky top-28">
            <div className="text-xl font-medium text-[#3a3f47]">Event List</div>
            <p className="text-sm font-light text-[#3a3f47] my-5">
              List of Ongoing Events
            </p>
          </div>
        </div>
        <div className="w-full md:w-[70%]">
          <Search />
          <hr className="my-4" />
          <div className="w-full flex justify-end items-center gap-5">
            <div className="block mb-2 text-md font-medium text-[#3a3f47]">
              Sort by:
            </div>
            <InputSelect />
          </div>

          <div className="grid grid-cols-1 relative z-10 md:grid-cols-2 gap-5 mt-10">
            {items.slice(0, 4).map((item, index) => (
              <div
                key={index}
                className="max-w-sm border bg-white rounded-lg shadow-2xl flex flex-col h-full"
              >
                <div
                  className="cursor-pointer"
                  onClick={() => handleCardClick(item.id)}
                >
                  <img
                    className="w-full h-full object-cover rounded-t-lg"
                    src={item.img}
                    alt=""
                  />
                </div>
                <div className="p-5 flex-grow">
                  <div className="flex gap-2 flex-wrap">
                    {item.category.map((category, index) => (
                      <span
                        key={index}
                        className="py-1 px-2 text-[#3a3f47] bg-transparent font-medium border-2 border-[#f2f2f2] rounded-md text-xs"
                      >
                        {category.name}
                      </span>
                    ))}
                  </div>
                  <div
                    className="cursor-pointer"
                    onClick={() => handleCardClick(item.id)}
                  >
                    <h5 className="mb-2 mt-3 text-xl font-normal text-blue-500 hover:underline">
                      {item.title}
                    </h5>
                  </div>
                  <p className="text-[10px] font-light text-[#3a3f47] mb-2">
                    by: {item.organizer} |{" "}
                    <span className="text-blue-500">{item.place}</span>
                  </p>
                  <p className="mb-3 text-sm font-light text-gray-400">
                    {item.description}
                  </p>
                </div>
                <div className="w-full px-5 py-3 bg-[#f2f2f2] flex justify-between">
                  <div className="text-xs text-[#3a3f47]">
                    Remaining Quota: {item.quota}
                  </div>
                  <div className="text-xs text-[#3a3f47]">{item.deadline}</div>
                </div>
              </div>
            ))}
          </div>

          <Pagination />
        </div>
      </div>
    </main>
  );
}

export default Page;

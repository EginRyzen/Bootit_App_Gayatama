        ...
        return (
          <div key={item.id} className="relative">
            {!item.submenu ? (
              <a
                href={item.link}
                className="block cursor-pointer hover:bg-gray-100 py-3 px-5 rounded-sm text-black transition-all duration-300 ease-in-out text-base md:bg-transparent font-medium"
              >
                {item.name}
              </a>
            ) : (
              <div className="group dropdown flex text-black hover:text-black cursor-pointer font-medium text-base tracking-wide hover:bg-gray-100 py-3 px-5 rounded-sm">
                <span>{item.name}</span>
                <div className="dropdown-menu w-[500px] absolute left-0 mt-5 pt-4 max-h-0 opacity-0 transform -translate-y-5 transition-all duration-500 ease-in-out overflow-hidden group-hover:max-h-96 group-hover:opacity-100 group-hover:translate-y-0">
                  ...
                  <div className="backdrop-blur-sm bg-white/80 border border-gray-200 rounded-lg shadow p-3">
                    ...
                    <ul className="w-auto grid grid-cols-2 gap-1">
                      {item.submenu.map((subItem) => (
                        <li
                          ...
                          key={subItem.id}
                          className="pl-3 pr-12 py-3 flex gap-1 items-center rounded-md hover:bg-[#e6e6e6]"
                        >
                          {subItem.icon}
                          <a
                            href={subItem.link}

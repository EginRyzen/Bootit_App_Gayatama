...
export default function Navbar() {
  ...
  const handleCategoryClick = (category) => {
    ...
    window.location.href = `/bootcamp/${category
      .replace(" ", "-")
      .toLowerCase()}`; // Ubah spasi menjadi dash dan kecilkan huruf untuk URL
  };
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            <div className="flex items-center">
              <div className="flex -ml-2">
                <a
                  href="/"
                  className="flex flex-col sm:flex-row items-center space-x-2 sm:space-x-0 rtl:space-x-reverse"
                >
                  <span className="text-[#508a8b] mb-0 sm:self-end text-xl sm:text-3xl font-bold whitespace-nowrap">
                    Boot.<span className="text-[#3a3f47]">IT</span>
                  </span>
                </a>
              </div>
            </div>
